...
app.config['SQLALCHEMY_DATABASE_URI']='sqlite:////var/www/hb/database.db'
...
with app.app_context():
    db.create_all()
    if db.session.query(User).filter_by(username='jc').count() < 1:
        db.session.add(User(
          username='jc',
          password=guard.hash_password('mikasico'),
          roles='admin'
		))
    db.session.commit()
